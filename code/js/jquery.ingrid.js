...
jQuery.fn.ingrid = function(o){
    ...
    if (cfg.paging) {
            ...
            pv = jQuery('<div />')
                        ...
                        .extend({
                            updateViewInfo : function(loaded_rows, page){
                                ...
                                //this.html('Viewing' + _start + ' - ' + _end + ' of ' + cfg.totalRecords);
                                this.html('total : ' + cfg.totalRecords);
                                return this;
                            }
                        });
        ...
        //var pfld  = jQuery('<input type="hidden" value="' + cfg.pageNumber + '"/><span>' + cfg.pageNumber + '</span>');
        
        var pn = jQuery('Page ' + cfg.pageNumber + ' of ' + totp);
        ...
        var relationship_radio         = jQuery('<div />')
                                .addClass(cfg.pageToolbarClass)
                                .height(cfg.pageToolbarHeight)
                                .width(b.width())
                                .extend({                                        
    ...
    var gap = jQuery('<div />').width(cfg.scrollbarW).addClass(cfg.headerClassB).height(cfg.headerHeight).css({
        position: 'absolute',
        zIndex: '0'
    }).appendTo(g);
    ...
    g.extend({
        ...
        returnSavedRowIds : function() {
            var id_string = '';
            if (jQuery.cookie) {
                id_string = jQuery.cookie( this.attr('id') + '_rows' );
                //alert(id_string);
            }            
            return id_string;
        },
