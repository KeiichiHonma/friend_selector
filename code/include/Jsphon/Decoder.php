...
/*    function decode($json)
    ...
    var $index_number = 0;
    var $api_setting_decoder = array();
    
    function decode($json,$api_setting,$is_multiquery)
    {
        $tknz = new Jsphon_Decoder_Tokenizer($json,
                                             array(&$this, 'handleInternalError'));
        $this->_internalError = false;
        ...
        if($tknz->nextToken()) {
            ...
            if(!$is_multiquery && count($api_setting) == 1){
                ...
                $result = $this->_decodeJSValueMulti($tknz);
            }else{
                $result = $this->_decodeJSValue($tknz);
            }
...
/*            if($is_multiquery){
                ...
                $result = $this->_decodeJSValueMulti($tknz);
            //マルチクエリじゃない
            }elseif(!$is_multiquery && count($api_setting) == 1){
                $result = $this->_decodeJSValueEx($tknz);
            //通常 index変更なし
            }else{
                $result = $this->_decodeJSValue($tknz);
            }*/
/*            if(!is_null($api_setting['index'])){
                $result = $this->_decodeJSValueEx($tknz,$api_setting['index']);
            }else{
                $result = $this->_decodeJSValue($tknz);
            }*/
    ...
    //インデックス変更拡張////////////////////////////////////////////////////////
    function _decodeJSValueEx(&$tknz)
    {
        //print 'test';
        switch($tknz->getToken()) {
          case JSPHON_TOKEN_DATUM:
          ...
          case JSPHON_TOKEN_LBRACKET:
            return $this->_decodeArrayEx($tknz);
            return $r;
    ...
    function _decodeArrayEx(&$tknz)
    {
        ...
        //if false, break
        while(  ($value = $this->_decodeJSValueEx($tknz) or true)
            and !$this->_internalError
            and ($ret[$value[$this->api_setting_decoder[$this->index_number]]] = $value or true)
            and $token = $tknz->nextToken()
            and $token == JSPHON_TOKEN_COMMA
            and $token = $tknz->nextToken()
            );
    ...
    function _decodeJSValueMulti(&$tknz)
    {
        //print 'test';
        switch($tknz->getToken()) {
          case JSPHON_TOKEN_DATUM:
          ...
          case JSPHON_TOKEN_LBRACKET:
            return $this->_decodeArrayMulti($tknz);
     ...
     * @return array
     */
    function _decodeArrayMulti(&$tknz)
    {
        ...
        //if false, break
        while(  ($value = $this->_decodeJSValueMulti($tknz) or true)
            and !$this->_internalError
            and ($ret[] = ( isset($this->api_setting_decoder[$this->index_number]) ? $this->changeIndex($value,$this->api_setting_decoder[$this->index_number]) : $value ) or true)
            and $token = $tknz->nextToken()
            and $token == JSPHON_TOKEN_COMMA
            and $token = $tknz->nextToken()
            );
    ...
    function changeIndex($array,$inedx){
        if( $inedx != '' ){
            foreach ($array['fql_result_set'] as $key => $value){
                unset($array['fql_result_set'][$key]);
                //flid対策。100004127376109_124542800973931というフォーマットでIDがふられる可能性があるためチェック
                if(is_numeric($value[$inedx])) $array['fql_result_set'][$value[$inedx]] = $value;
            }
        }
        $this->index_number = $this->index_number + 1;
        return $array;
    }
